...
namespace caffe {
template <typename Dtype> // slow implement O(n^3) //only minibathc
__global__ void print_backward_gpu(int nthreads,const Dtype * weight_diff,const Dtype* label,
                                   const int K_) {
  const int label_value = static_cast<int>(label[1]);
  printf("weight_diff:%f \n",weight_diff[label_value * K_ + 1]);
}
...
__global__ void Weight_mean_gpu(int nthreads, const int K_, const int N_,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    for (int i = 0; i < K_; i++) {
      weight_mean[i] += weight[index * K_ + i] / N_ ;
    }
  }
}
...
void MarginInnerProductLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    //int nthreads = N_;
    //print_backward_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
    //  CAFFE_CUDA_NUM_THREADS>>>(nthreads,this->blobs_[0]->gpu_diff(),label,K_);
